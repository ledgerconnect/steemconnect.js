...
var SteemConnect = function(){};

SteemConnect.prototype.isValid = function(name, password, auths){
	var hasKey = false;
	var roles = [];
...
SteemConnect.prototype.generateKeys = function(name, password, roles){
	var pubKeys = {};
	roles.forEach(function(role){
		...
		var string = 'STM' + bs58.encode(addy);
		pubKeys[role] = string;
	});
	return pubKeys;
};
