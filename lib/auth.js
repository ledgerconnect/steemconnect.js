var bigi = require('bigi'),
	...
	transaction = require("./operations").transaction;
...
Auth.signTransaction = function(tx, keys) {
	var signatures = [];
	var chain_id = '';
	for (var i = 0; i < 32; i++) {
		chain_id += '00';
	}
	var cid = new Buffer(chain_id, 'hex');

	// Ok Till here
	var buf = transaction.toBuffer(tx);
	for (var key in keys) {
		var sig = Signature.signBuffer(Buffer.concat([cid, buf]), key);
		signatures.push(sig.toBuffer())
	}

	return {tx: tx, signatures: signatures};
};
